...
final class PalmosSMARTService: NSObject, PalmosSMARTXPCProtocol, @unchecked Sendable {
    ...
    func readSMARTData(
        ...
        do {
            request = try PalmosXPCMessages.decodeSMARTReadRequest(from: requestData)
        } catch {
    ...
    func cancelSMARTDataRequest(
        ...
        let replyBox = XPCReplyBox(reply)
        do {
            let request = try PalmosXPCMessages.decodeSMARTCancelRequest(from: requestData)
            ...
            replyBox.call(
                try PalmosXPCMessages.encodeSMARTCancelAcknowledgement(acknowledgement),
                nil
            )
        } catch {
            replyBox.call(nil, error as NSError)
    ...
    func installSmartctlCompanion(
        ...
        Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                result = (
                    try PalmosXPCMessages.encodeSMARTCompanionInstallAcknowledgement(
                        acknowledgement
                    ),
                    nil
                )
...
private final class SMARTReadOperation: @unchecked Sendable {
    ...
    private func run() async {
        ...
        do {
            ...
            switch mode {
            case .legacy:
                replyBox.call(PalmosXPCMessages.legacySMARTReply(payload: payload), nil)
            case .completionAware:
                let response = SMARTReadCompletionResponse(
                    schemaVersion: SMARTReadCompletionResponse.currentSchemaVersion,
                    payload: payload,
