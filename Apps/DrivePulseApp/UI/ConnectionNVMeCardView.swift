...
struct ConnectionNVMeCardView: View {
    ...
    private func thunderboltBlock(_ tb: ThunderboltInfo) -> some View {
        Grid(alignment: .leading, horizontalSpacing: 12, verticalSpacing: 6) {
            ...
            row("Bus", PanelDisplayValue.string(tb.bus.map { "Bus \($0)" }))
            row("Receptacle", PanelDisplayValue.string(tb.receptacle.map { "Receptacle \($0)" }))
    ...
    private var nvmePCIBlock: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Grid(alignment: .leading, horizontalSpacing: 12, verticalSpacing: 6) {
                ...
                row("TRIM", device?.nvmeInfo?.trimSupport.map { $0 ? "Yes" : "No" } ?? PanelDisplayValue.missing)
                ...
                row("Firmware No-Reset Update", device?.nvmeInfo?.firmwareUpdateRequiresReset.map { $0 ? "No" : "Yes" } ?? PanelDisplayValue.missing)
                ...
                row("Tunnel Compatible", device?.pciInfo?.tunnelCompatible.map { $0 ? "Yes" : "No" } ?? PanelDisplayValue.missing)
