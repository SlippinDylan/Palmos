...
final class HelperOccupancySecurityTests: XCTestCase {
    func testSMARTXPCSelectorNamesRemainByteStable() {
        XCTAssertEqual(
            NSStringFromSelector(#selector(PalmosSMARTXPCProtocol.fetchHelperHandshake(withReply:))),
        ...
        XCTAssertEqual(
            NSStringFromSelector(#selector(PalmosSMARTXPCProtocol.cancelSMARTDataRequest(for:withReply:))),
            "cancelSMARTDataRequestFor:withReply:"
        )
        XCTAssertEqual(
            NSStringFromSelector(#selector(PalmosSMARTXPCProtocol.installSmartctlCompanion(for:withReply:))),
    ...
    func testCompletionAwareSMARTReturnsBoundedBusinessErrorEnvelopes() async throws {
        ...
        for (runnerError, expectedCode) in cases {
            let service = PalmosSMARTService(runner: FixtureSMARTDataRunner(result: .failure(runnerError)))
            let requestID = UUID()
            ...
            let response = try PalmosXPCMessages.decodeSMARTReadCompletionResponse(from: replyData)
    ...
    func testCompletionAwareSMARTQuiescenceDistinguishesAdmittedAndRejectedRequests() async throws {
        ...
        let successService = PalmosSMARTService(
        ...
        let decodedLegacy = try PalmosXPCMessages.decodeSMARTReadCompletionResponse(
    ...
    func testBoundedSMARTCancellationAcknowledgesAndCompletionObservesExit() async throws {
        ...
        XCTAssertThrowsError(
            try PalmosXPCMessages.decodeSMARTCancelRequest(
    ...
    func testHandshakeReportsCompanionCapability() async throws {
        let unavailable = PalmosSMARTService(
        ...
        let handshake = try PalmosXPCMessages.decode(
    ...
    func testSMARTRequestAndPayloadLimitsRejectOversizedMessages() throws {
        ...
        XCTAssertThrowsError(try PalmosXPCMessages.encodeSMARTReadRequest(oversizedRequest))
        ...
        XCTAssertThrowsError(try PalmosXPCMessages.encodeSMARTReadRequest(invalidRequestID))
        ...
        XCTAssertThrowsError(try PalmosXPCMessages.encodeSMARTReadCompletionResponse(response))
        XCTAssertThrowsError(try PalmosXPCMessages.decodeSMARTReadCompletionResponse(from: oversizedPayload))
    }
    ...
    func testSMARTCompanionInstallMessagesAreBoundedAndSchemaChecked() throws {
        ...
        let encoded = try PalmosXPCMessages.encodeSMARTCompanionInstallRequest(request)
        XCTAssertEqual(
            try PalmosXPCMessages.decodeSMARTCompanionInstallRequest(from: encoded),
            request
        )

        XCTAssertThrowsError(
            try PalmosXPCMessages.encodeSMARTCompanionInstallRequest(
                .init(binary: Data())
            )
        )
        XCTAssertThrowsError(
        ...
        XCTAssertThrowsError(
            try PalmosXPCMessages.decodeSMARTCompanionInstallRequest(
        ...
        let acknowledgementData = try PalmosXPCMessages.encodeSMARTCompanionInstallAcknowledgement(
            acknowledgement
        )
        XCTAssertEqual(
            try PalmosXPCMessages.decodeSMARTCompanionInstallAcknowledgement(from: acknowledgementData),
            acknowledgement
        )
    }

    func testSMARTCompanionDigestIsLowercaseAndStable() {
        let data = Data("Palmos".utf8)
        XCTAssertEqual(
            SecuritySMARTCompanionCodeValidator.sha256Hex(data),
            "9aed278b90f2d1156e60de4e31c5a6fb819a85b21fd1bb1df455603b79b417a4"
        )
        XCTAssertTrue(
    ...
    func testCompanionInstallEndpointPassesOnlyBoundedBinaryAndAcknowledges() async throws {
        ...
        let requestData = try PalmosXPCMessages.encodeSMARTCompanionInstallRequest(
            .init(binary: binary)
        )
    ...
    func testCompanionInstallerRejectsWritableOrSymlinkDestinationDirectory() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PalmosCompanionSymlink-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCompanionSigningRelationshipRejectsAdHocIdentifierAndTeamMismatches() {
        let helper = SMARTCompanionSigningIdentity(
            identifier: "com.palmos.smartservice",
            teamIdentifier: "TEAM",
            isAdHoc: false
        )
    ...
    private func makeExecutableFixture(
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PalmosSmartctlTests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeCompanionInstallFixture() throws -> (
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PalmosCompanionInstall-\(UUID().uuidString)", isDirectory: true)
    ...
    private func completionResponse(
        ...
        XCTAssertNil(reply.error)
        return try PalmosXPCMessages.decodeSMARTReadCompletionResponse(
            from: XCTUnwrap(reply.data)
        )
    }
    ...
    func testResponseSchemaContainsNoSensitiveFields() throws {
        ...
        let object = try XCTUnwrap(JSONSerialization.jsonObject(with: PalmosXPCMessages.encodeOccupancyResponse(response)) as? [String: Any])
    ...
    func testEndpointRejectsOversizedBytesBeforeDecodeAndMapsBoundedError() async {
        ...
        XCTAssertEqual(result.error?.domain, "com.palmos.smartservice.occupancy")
    ...
    func testEndpointEncodesBoundedOccupancyResponse() async throws {
        ...
        let request = try PalmosXPCMessages.encodeOccupancyRequest(
        ...
        XCTAssertEqual(try PalmosXPCMessages.decodeOccupancyResponse(from: data).workflowID, workflowID)
    }
    ...
    func testSameWorkflowNewGenerationCancelsWithoutStartingConcurrentWorker() async throws {
        ...
        XCTAssertFalse(
            try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(newResult.data)).isComplete
        )
        let oldResult = await old.value
        let oldResponse = try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(oldResult.data))
    ...
    func testEndpointReturnsWithinFullDeadlineEvenWhenSnapshotWorkerIgnoresCancellation() async throws {
        ...
        let response = try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(result.data))
    ...
    func testSameWorkflowSupersedeTerminatesTopologyProcess() async throws {
        ...
        XCTAssertFalse(
            try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(newer.data)).isComplete
        )
        XCTAssertLessThan(start.duration(to: clock.now), .seconds(1))
    }
    ...
    func testDeadlineReplyKeepsSlotOwnedUntilNoncooperativeWorkerActuallyExits() async throws {
        ...
        XCTAssertFalse(try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(first.data)).isComplete)

        for _ in 0..<5 {
            let repeated = await endpoint.handle(request)
            XCTAssertFalse(try PalmosXPCMessages.decodeOccupancyResponse(from: XCTUnwrap(repeated.data)).isComplete)
        }
        let other = await endpoint.handle(try occupancyRequest(UUID()))
        XCTAssertEqual(other.error?.code, HelperOccupancyError.helperBusy.rawValue)
...
private func occupancyRequest(_ workflowID: UUID, disk: String = "disk4") throws -> Data {
    try PalmosXPCMessages.encodeOccupancyRequest(
...
private final class SMARTReplyProbe: @unchecked Sendable {
    ...
    func start(service: PalmosSMARTService, request: SMARTReadRequest) {
        let requestData = try! PalmosXPCMessages.encodeSMARTReadRequest(request)
