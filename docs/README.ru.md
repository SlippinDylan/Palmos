...
| Минимальная версия | macOS 15 или новее |
...
Каждый GitHub Release содержит один файл `Palmos-v<версия>.dmg`. Откройте DMG и перетащите `Palmos.app` в `Applications`. Выпуски подписываются бесплатным сертификатом Apple Development, но не проходят нотариальное заверение. Перед первым запуском удалите атрибут карантина:
