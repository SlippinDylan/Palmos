...
test('requires exactly the version and release manifest fields', () => {
  assert.equal(parseManifest('{"version":"1.2.3","release":false}').release, false);
...
test('extracts only the exact non-empty changelog section', () => {
  assert.equal(
    extractChangelogSection(changelog, '1.1.0-beta.1'),
    '### Added\n\n- Added release automation.',
  );
  assert.throws(() => extractChangelogSection(changelog, '1.1.0-beta.2'));
...
test('rejects versions that do not advance published history', () => {
  assert.doesNotThrow(() => validateReleaseOrder(parseVersion('1.1.0-alpha.1'), ['v1.0.3']));
  assert.throws(() => validateReleaseOrder(parseVersion('1.0.3'), ['v1.0.3']));
  assert.throws(() => validateReleaseOrder(parseVersion('1.0.2'), ['v1.0.3']));
});
...
test('requires matching notes and advancing history when release is enabled', () => {
  const plan = createReleasePlan({
    manifestSource: '{"version":"1.1.0-beta.1","release":true}',
    changelogSource: changelog,
    publishedTags: ['v1.0.3'],
  });
  assert.equal(plan.release, true);
  assert.throws(() => createReleasePlan({
    manifestSource: '{"version":"1.1.0-beta.2","release":true}',
    changelogSource: changelog,
    publishedTags: ['v1.0.3'],
  }));
});
